 ...
 * (probe timeout + agent overhead + margin), not an unrelated constant, so the
 * lock cannot lapse mid-dispatch.
 *
 * Queue policies:
...
class QueuePolicyManager(private val redis: RedisCommands<String, String>) {
    ...
    private fun lockTtlSeconds(timeoutMs: Int): Long {
        val ceilingMs = timeoutMs.toLong() + DISPATCH_OVERHEAD_MS + SAFETY_MARGIN_MS
